...
- **석호의 계단** — 아무것도 안 꺼냈다. 다운로드량과 매체 노출만 봤다
...
③(심사 항목) 8개. 규정마다 성격이 달라 고르게 나눌 이유가 없다.

**공유 팩트는 판정 함수에 명시적으로 안 들어간다.** 표면 인상이 else 분기의 기본값으로
접혀 있기 때문이다 — 01 은 연출·완성도 기본값이 석호인 근거, 02 는 설계 기본값이 종이
낚시인 근거, 03 은 밤길이 기본값에서 빠지는 근거, 04 는 완성도 기본값을 굳히는 근거다.
그래서 "팩트를 빼면 답이 바뀌는가" 검사에 공유는 안 걸린다. 실제 에이전트는 그 문장을
읽고 판단하므로 문면은 여전히 일한다.
...
**prior 프로브는 끝났다** — 2026-08-19, `probe_prior_new.py`, 실호출 12콜,
**known 0/12 · 파싱 실패 0**. 게임·시상식은 모델이 아는 도메인이라 다른 재료보다 오염
위험이 컸는데, 이름을 전부 지어낸 덕에 앞선 재료 넷과 같은 0/12 를 받았다. camp 과
같은 문면(`camp-prior-v0.2`)을 썼으므로 camp 의 0/12 와 나란히 놓을 수 있다.
응답 원문은 `prior_issue_award.json` 에 전량 보존돼 있다.
...
FACTS = [
    ...
    # 04 는 종전에 "세 작품 모두 예비 검토를 통과했다"였는데 어느 심사 항목의 근거도
    # 아니어서 붕 떠 있었다(2026-08-19 요한 교정). 발매 연기로 바꾸면 완성도 항목의
    # 표면 인상을 석호 쪽으로 미는 근거가 되고, **팩트 11 과 짝이 된다** —
    # 연기는 표면상 불안 신호인데 11(넉 달간 치명 결함 0건)이 나오면 그래서 다듬은
    # 것이었다는 해석으로 뒤집힌다. 취합이 해석을 뒤집는 자리를 하나 더 만든다.
    (4, "밤길 안내인은 발매를 한 차례 연기한 적이 있다",
     3, "shared", DECOY, False, True, r"연기"),
    # ── 미공유 8개: 전부 critical ──
    # 석호의 계단 결격 — 두 조각이 만나야 성립한다.
    # 05 는 "안 썼다"는 신고, 06 은 "참고로 썼다"는 발언. 둘이 어긋나야 미신고가 성립한다.
    # 06 을 "배경 미술 대부분을 만들었다"로 두면 결격이 너무 노골적이라 참고 수준으로 낮췄다
    # (2026-08-19 요한 교정). 신고가 '쓴 적 없다'이므로 참고 사용만으로도 어긋난다.
    (5, "석호의 계단 제작 신고서에는 생성 도구를 쓴 적이 없다고 적혀 있다",
     2, "unshared", ANSWER, True, None, r"쓴\s*적이\s*없다고"),
    (6, "기슭 스튜디오 대표는 배경 초안을 자동 생성 도구로 뽑아 참고했다고 인터뷰에서 말했다",
    ...
    # 설계 항목 — 종전에는 "조작 설계가 표준 규격으로 채택됐다"였으나 그런 제도가 없어
    # 업계에서 애매한 사례라는 지적을 받아 이탈률로 바꿨다(2026-08-19 요한 교정).
    (10, "밤길 안내인은 안내 문구를 한 줄도 쓰지 않고 첫 관문 이탈률이 셋 가운데 가장 낮다",
     3, "unshared", ANSWER, True, None, r"이탈률"),
    (11, "밤길 안내인은 출시 뒤 넉 달 동안 치명 결함 보고가 한 건도 없다",
     3, "unshared", ANSWER, True, None, r"넉\s*달"),
    # 석호의 계단 표면 강점을 깎는 사실.
    # 판매량이면 무료 배포분을 합칠 수 없어 앞뒤가 안 맞았다 — 다운로드량으로 정정.
    (12, "석호의 계단의 다운로드량 1위는 무료 배포분을 합친 수치다",
...
def verdict(known: set[int]) -> str:
    ...
    ③ 남은 작품 가운데 심사 세 항목의 승자를 세고, 많이 가진 쪽이 수상한다.
       각 항목의 승자는 밤길의 숨은 사실이 나오기 전에는 표면 인상이 정한다.
         ...
         완성도 — 팩트 11 이 나오면 밤길, 아니면 석호(다운로드 1위라 안정적으로 보인다)
