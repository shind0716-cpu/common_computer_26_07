"""[재료 확장 · 김요한] prior 프로브 — 새 재료 3종의 팩트가 모델의 사전지식에 있나 (실호출)

camp 은 팩트 12개를 확정한 뒤 "이 특정 사실을 이미 아는가"를 물어 known 0/12 를 확인하고서야
사연을 썼다(`probe_prior.py` 의 ms1). 그 관문을 새 재료 3종에도 건다.
...
실행: `PYTHONUTF8=1 python experiments/scenario_generalization/probe_prior_new.py`
산출: `prior_<issue>.json` 3종 + facts 파일의 `prior` 칸 기입
"""
