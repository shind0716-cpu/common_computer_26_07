"""3종 시나리오 공통 — 배분 자동 생성(제약 만족) + 골격 자체 검사.

골격 규격(v4-16):
  팩트 16 · 8에이전트(관점4 x 입장2) · 에이전트당 4팩트 · 밀도 2.0
  관점 4분할(관점당 4팩트) · 자기 관점 2 + 타 관점 2 · 각 에이전트 자기입장 불리 1+ 보유
  고립(1인 보유) 정확히 3 · 미배분 0 · 최대 보유 3 · 불리 pro:con 대칭 · 본문 팩트 노출 0
"""
import json, random
...
def gen_assignment(facts, iso_target=3, seed=42, tries=500000):
    """제약을 만족하는 배분을 랜덤 탐색해서 반환. 실패 시 None."""
    by_persp = {}
    for f in facts:
...
def check(issue, facts, assign, expect_self=2, expect_other=2, iso_target=3):
    """골격 자체 검사. (ok, 리포트dict) 반환."""
    F = facts['facts']
...
def write_docs(base, iid, title, body, note_issue, note_facts, facts, agents, prompt_ver="scenario-cand-v4-16"):
    base = Path(base)
    ...
    docs = {
        ...
        base/"assignments"/f"assignment_{iid}.json": {**common,
            "_note": "8에이전트(관점 4 x 입장 2), 에이전트당 4팩트, 밀도 2.0, 자기2/타2, 각 자기입장 불리 1+, 고립 3.",
