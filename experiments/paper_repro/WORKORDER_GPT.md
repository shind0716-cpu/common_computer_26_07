...
# §12. recall probe 재작업 — 검수 반영 + 시계열 확장 (2026-08-11 요한 승인, append)

§11 구현은 검수 통과(Claude 재현: 테스트 54/311·dry 일치·관문 표 줄 번호 확인).
아래 4건을 반영 후 재검수 요청할 것.

1. **F-1 (차단급 수정)**: parse 실패 1건이 전체 중단 + 캐시 적중 영구 재현되는 구조 제거.
   행 단위 `parse_status: ok|parse_fail` 제3상태로 기록하고 계속 진행(부재≠소실 —
   bridge·E-1 수리와 같은 원칙). parse_fail 행도 pre_mapping 에 raw 참조와 함께 남긴다.
   회귀 테스트: 실패 1건 섞인 48행 → 47 파싱 + 1 parse_fail + 파일 작성 성공.
2. **시계열 확장 (요한 확정 범위)**: `probe_full` 은 **전 시점**(r0..최종, 라운드별 입력
   재조립 — 기존 최종 시점 경로의 일반화), `probe_note` 는 **최종 시점만**(시점별 note 는
   협력 트랙 수첩 실험 선점이라 제외 — 경계 유지). tag 에 round 포함, 행에 `probe_round`.
   **G1 갱신: full 8×4시점×3건=96 + note 8×1×3=24 = 120콜, 상한 144.** 근거: 소급 프로브는
   무상태 모델이라 시점 관측과 분포 동등·토론 오염 0(완주 기록 위 재구성).
3. **구조화 출력 확인 (수정 금지·확인만)**: `modules/llm.py` 의 `obtain_response` 가
   `response_format`(json_schema) 을 지원하는지 코드로 확인해 보고할 것. 지원 시 적용
   제안서(스키마 정의 포함), 미지원 시 **수정하지 말고** 인계 문안(동범 소관) 작성 —
   G2(주장마다 코드 줄 번호) 적용.
4. **검수 결정 반영**: ① `modules/paths.py` 추가 2함수는 수용(요한 8/11 — 보드 통지 예정,
   추가 수정도 같은 수용 범위) ② pre_mapping 산출물 위치를 `data/judgments/` →
