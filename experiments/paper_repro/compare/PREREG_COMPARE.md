...
## 10. 판독 규칙 append — 셀 차이 원문 판독 절차 (R2, 2026-08-12 · 원문 판독 전 고정)

§3-2 의 "차이 관찰은 원문 판독 뒤에만 원인 후보 기술"을 이행하는 절차. **이 규칙 커밋
후에 판독한다** — 결과를 보고 규칙을 만들면 사후 해석이 된다.

1. **차이 셀 식별 (기계적)**: 같은 stage 에서 저자 판 mentioned 집합과 우리 판 mentioned
   집합의 대칭차. 방향 표기: `저자만` (저자 mentioned·우리 unmentioned) / `우리만`.
2. **언급 판 검증**: mentioned 로 판정된 판의 발화 원문에서 해당 팩트의 근거 구절을
   직접 인용 확인(판정 원장 raw 의 matched 근거). 구절을 못 찾으면 "판정 근거 미확인"
   으로 적고 원인 기술을 보류한다.
3. **미언급 판 훑기**: 미언급 판의 그 stage 발화 전량(8건)에서 해당 팩트의 유사 표현
   유무를 훑는다(사람 읽기 — 자동 매칭 금지).
4. **기술 어휘 한정 (두 범주만)**:
   - `발화 부재` — 미언급 판 발화에 관련 표현이 없음 → 엔진(발화 생성) 차이 후보
   - `판정 경계` — 유사 표현이 있으나 unmentioned 판정 → 판정기 경계 사례 후보
   원인 단정·"재현" 어휘 금지. 결론 어휘는 "구현 등가성 ± 마진" 틀에 종속.
5. 기록: MUTUAL_REVIEW append (팩트 원문·인용 구절·범주·양판 발화 참조 좌표 포함).

---

## 11. 실행 편차·축소 append (2026-08-12 실행분 — RESULT §4·§5 의 정본 등재)

- **P-10** 저자 키 자리 일시 주입·원상 복구(sha 70a3f15290dbd10b 왕복 검증) ·
  **P-11** gpt-5 온도 미전송(API 400 강제 — 양판 대칭, 실효 판정 온도=공급자 기본값) ·
  **P-12** 우리 판 이슈별 프로세스 분리(llm 온도 폴백 플래그 전역성 회피 — §8 트랜치
  1명령을 3회 분할로 운영 변경) · **P-13** probe 토큰 천장 24576(절단 2회 실측).
- **probe 축소(요한 8/12)**: §6 의 120콜(3이슈) → **0543 1이슈 40콜**로 축소 실행.
  근거: 2인 패널 중간 판독에서 계기 정합(출처 시계열·자발 압축)과 계기 함정(지시문
  혼입·raw 개수 비지표)이 동시에 확인돼, 매핑 판정 설계 전 3이슈 확장의 실익 없음.
  0248·0262 probe 는 미실행(부재≠소실) — 후속 실험에서 매핑 설계와 함께.
- 저자 판 명령열 실행 중 발견(§6 M-1 보완): 저자 코드는 `content/`·`evaluate_res/`
  디렉터리 존재를 가정한다(부재 시 mkdir 실패 — 사전 생성 필요). 재시도 부재로
  네트워크 순단 1회에 크래시 — 감독 재실행(진행 가드)으로 완주.
