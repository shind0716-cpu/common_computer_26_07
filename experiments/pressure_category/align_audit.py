...
def main() -> int:
    ...
    L += ["", "## ② 판독 — READ60 이 현행 재료를 가리키는가", "",
          ...
          "(옛 판은 `runs/_stale_hash/` 에 보존) **판독만 옛 재료 기준**이다. 다시 읽기 전에는",
          "이 여덟 벌로 뜻 기준 수치를 낼 수 없다.",
    ...
    L += ["",
          ...
          "## 그래서 무엇을 쓸 수 있나", "",
          "| 산출물 | 근거 | 판정 |", "|---|---|---|",
          "| `SELECTION_live30` §23 (H0·H3′·H6 판정) | 8/27 60판 + 판독 1,424칸 | **조건부** — 30쌍 중 8쌍이 개정 전 재료. 22쌍 값으로 다시 적어 인용한다 |",
          "| READ60 판독 원장 · κ 0.817 | 같음 | **조건부** — 22벌 유효, 8벌은 다시 읽어야 한다 |",
