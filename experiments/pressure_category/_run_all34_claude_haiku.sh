#!/usr/bin/env bash
# 3모델 ALL 균형 — claude-haiku 따라잡기 (런플랜 ALL34_3MODEL_2026-09-01)
# 배치 4개 · 판 36 · 예상 콜 288 (판당 상한 11)
# 중단·재개: 이 스크립트를 그대로 재실행하면 완료 판은 러너가 [skip], 미완 판은
#            .partial.jsonl 체크포인트에서 이어받는다.
# 배치 하나만 돌리려면: BATCH=2 bash _run_all34_claude_haiku.sh
set -u
LOG=runs/all34_claude_haiku_2026-09-01.log
echo "=== ALL34 claude-haiku 시작 $(date -Iseconds) ===" | tee -a "$LOG"
consec_fail=0
run_one() {  # $1=배치 $2=재료 $3=세트 $4..=반복
  local b=$1 id=$2 vs=$3; shift 3
  echo "--- [배치 $b] $id / $vs / rep $* ---" | tee -a "$LOG"
  PYTHONUTF8=1 python run_pressure.py --materials "$id" --model claude-haiku     --scripts C0 --vsets "$vs" --reps "$@" --max-calls 11 --allow-live 2>&1 | tee -a "$LOG"
