...
## 정정 1 (2026-09-03, **판독 뒤**) — 압박 대사를 한 줄로 접었다

아래 `본 = sorted(set(...))` 이 **똑같은 압박 대사 세 줄을 한 줄로 묶었고**,
그것을 「시간 순」 목록 **밖에** 실었다. 그래서 팩만 보면 압박이 한 번 들어온 것처럼
읽힌다. 실제로는 37판 전부 `script_lines` 가 세 줄이고 라운드마다 같은 말이 들어간다.

대조 에이전트가 팩 02·04·07 에서 이걸 잡았다 — 「압박이 몇 번 어느 자리에 들어왔는지는
팩에 적혀 있지 않다」. 그런데 판독문 여럿은 「압박 한 번에 전부 넘어간다」로 읽었다.
**팩이 가린 것을 읽는 쪽이 짐작으로 메운 것이고, 그 짐작은 틀렸다.**

무엇이 영향받나 —
- **틀린 것**: 「압박 한 번에 넘어간다」류의 말. 압박은 매 라운드 들어갔다
- **안 틀린 것**: 어느 발화에서 갈리는지, 무엇을 근거로 드는지, 최종 답이 돌아오는지.
  이 셋은 팩에 실린 칸만 보고 말한 것이라 압박 횟수와 무관하다

**이미 읽힌 팩은 안 고친다**(러너·팩은 증거물이다). 다시 지을 때만 아래가 바뀐다 —
압박 대사를 접지 말고 **라운드마다 시간 순 안에** 넣어야 한다.
"""
import json
import os
import pathlib
