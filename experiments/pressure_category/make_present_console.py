...
<div class="card"><h3>시나리오별 — 압박 없는 판에서 문장이 답을 갈랐나</h3><div id="scen"></div><div class="cap">칸 = 3반복의 마지막 답(① = 첫 선택지, ② = 둘째 선택지, ? = 판독 불가). ★ = 두 문장이 서로 다른 답(각 2/3 이상). 원칙은 문장이 하나라 갈림 자체가 없다.</div></div>
</section>
...
<select id="bf_set"><option value="">세트 전부</option><option>가치 A</option><option>가치 B</option><option>신념 보수</option><option>신념 진보</option><option>원칙</option></select><span class="sub" id="bcount"></span></div>
...
function three(){const rs=R.filter(r=>mods().includes(r.model)&&(!fi.value||r.issue===fi.value));const c=conds();const items=[];for(const k of c){const c0=rs.filter(r=>r.g===k&&!r.press),pr=rs.filter(r=>r.g===k&&r.press);
 ...
 let h='<div class="row" style="grid-template-columns:repeat(3,1fr)">';for(const [t,st,fn,fmt,mx] of panels){const W=300,H=170,bw=52;h+=`<div><div class="sub"><b>${t}</b><br>${st}</div>${svg(W,H)}`;items.forEach((it,i)=>{const v=fn(it);const x=30+i*90;if(isNaN(v)){h+=`<text x="${x+bw/2}" y="${H-30}" font-size="11" fill="#898781" text-anchor="middle">${it.k.endsWith('원칙')&&t==='답을 가르나'?'문장 하나':'–'}</text>`}else{const hh=Math.max(0,Math.min(1,Math.abs(v)/mx))*(H-60);const y=v>=0?H-30-hh:H-30;h+=`<rect x="${x}" y="${y}" width="${bw}" height="${hh}" fill="${COL[it.k]}" rx="3"/><text x="${x+bw/2}" y="${(v>=0?y:y+hh)-6+(v>=0?0:16)}" font-size="12" font-weight="600" text-anchor="middle" fill="#0b0b0b">${fmt(v)}</text>`}h+=`<text x="${x+bw/2}" y="${H-12}" font-size="11" fill="#52514e" text-anchor="middle">${it.k}</text>`});h+=`<line x1="20" x2="${W}" y1="${H-30}" y2="${H-30}" stroke="#c3c2b7"/></svg></div>`}h+='</div>';document.getElementById('three').innerHTML=h}
// ---- curve
...
// ---- scenario table
function scen(){const ms=mods();const rs=R.filter(r=>!r.press&&ms.includes(r.model));const sets=['가치 A','가치 B','신념 보수','신념 진보','원칙'];let h='<table><tr><th>시나리오</th><th>선택지</th>'+(ms.length>1?'<th>모델</th>':'')+sets.map(s=>`<th>${s}</th>`).join('')+'<th>가치 갈림</th><th>신념 갈림</th></tr>';
 for(const iid of Object.keys(M))for(const mo of ms){const m=M[iid];const ab=o=>o===m.options[0]?`<span class="pill" style="background:${OC(m,o)}">①</span>`:o===m.options[1]?`<span class="pill" style="background:${OC(m,o)}">②</span>`:'<span class="pill" style="background:#c9c8c1">?</span>';h+=`<tr><td><b>${m.ko}</b></td><td class="kv" style="font-size:11.5px">① ${m.options[0]}<br>② ${m.options[1]}</td>`+(ms.length>1?`<td style="color:${MCOL[mo]||'#2a78d6'};font-weight:600">${mo}</td>`:'');const maj={};sets.forEach(s=>{const gg=rs.filter(r=>r.issue===iid&&r.model===mo&&r.set===s);const cnt={};gg.forEach(r=>cnt[r.final]=(cnt[r.final]||0)+1);const top=Object.entries(cnt).sort((a,b)=>b[1]-a[1])[0];maj[s]=top&&top[1]>=2?top[0]:null;h+=`<td style="white-space:nowrap">${gg.map(r=>ab(r.final)).join(' ')||'–'}</td>`});const sp=(a,b)=>maj[a]&&maj[b]&&maj[a]!==maj[b]?'★':'';h+=`<td>${sp('가치 A','가치 B')}</td><td>${sp('신념 보수','신념 진보')}</td></tr>`}
 document.getElementById('scen').innerHTML=h+'</table>'}
// ---- browser
...
document.querySelectorAll('.fm,.fc,.fp,#fi,#bf_cls,#bf_set').forEach(e=>e.addEventListener('change',all));
