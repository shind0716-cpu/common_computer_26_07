...
<details open style="margin:6px 0 10px;font-size:12.5px;color:#52514e;background:#f7f6f2;border-radius:8px;padding:8px 12px"><summary style="cursor:pointer;font-weight:600;color:#0b0b0b">이 화면 읽는 법</summary>
<div style="margin-top:6px;line-height:1.55">
한 판 = 에이전트 하나가 같은 상대와 4라운드 이야기하고 마지막 답을 낸 기록. 라운드가 끝날 때마다 에이전트는 <b>500자 수첩</b>을 스스로 새로 쓰고, 다음 라운드는 그 수첩만 보고 시작한다(수첩 3장: r0·r1·r2).<br>
<b>왼쪽 목록</b>: 시나리오 · 모델 · 문장 세트 · 압박 여부 · (압박 판이면) 어떻게 끝났나 5분류 · <b>사실 r0→r1→r2</b> = 처음 준 사실 12개 중 수첩 세 장에 각각 알아볼 수 있게 남은 개수. 예: 9→3→1 = 첫 수첩엔 9개, 둘째엔 3개, 마지막엔 1개.<br>
<b>오른쪽 위 띠</b>: 글 4편이 각각 어느 쪽을 편들었나 + 마지막 답. 색 = 선택지(보라 = 첫 선택지, 주황 = 둘째). "모호" = 결론 없음.<br>
<b>격자</b>: 행 = 사실 12개(왼쪽 색점 = 그 사실이 유리한 선택지, 회색 글씨 = 항목), 열 = 수첩 3장. <span class="sw" style="background:#2a78d6"></span>있음 = 내용을 알아볼 수 있게 남음(말을 바꿔 써도 인정) · <span class="sw" style="background:#cde2fb"></span>접힘 = 항목 이름만 남고 내용은 없음 · 빈 칸 = 흔적 없음.<br>
격자가 <b>통째로 비어 있어도 수첩이 빈 게 아니다</b> — 수첩은 있는데(아래 원문 펼치면 300~500자) 사실 대신 원칙·결론만 적혀 있는 판이다. 하이쿠에서 흔하다.<br>
판 하나가 이상하면 판독 메모(소넷 채점자가 남긴 한 줄)와 수첩·글 원문을 펼쳐 직접 확인할 것. 채점은 LLM이 한 것이라 틀릴 수 있다.
</div></details>
<div class="row" style="grid-template-columns:minmax(0,2fr) minmax(0,3fr)"><div><div style="display:flex;gap:8px;padding:4px 10px;font-size:11px;color:#898781"><span style="width:12px"></span><span style="width:104px">시나리오</span><span id="bh_model" style="width:44px"></span><span style="width:64px">세트</span><span style="width:48px">압박</span><span style="flex:1">5분류</span><span>사실 r0→r1→r2</span></div><div class="list" id="blist"></div></div><div id="bdetail"><div class="sub">왼쪽에서 판을 고르면 입장 흐름 · 사실 격자 · 수첩 원문이 여기 나온다.</div></div></div>
</div>
</section>
...
function bdetail(){const r=R.find(x=>x.id===cur);if(!r)return;const m=M[r.issue];const opts=m.options;const oc=o=>OC(m,o);
 ...
 h+='<div class="kv" style="margin-top:6px;color:#898781;font-size:11.5px">글 4편이 각각 편든 쪽 → 마지막 답 (색 = 선택지)</div><div class="strip">'+r.stance.map((s,i)=>`<div class="st" style="background:${oc(s)}">라운드 ${i} 글<b>${s}</b></div>`).join('')+`<div class="st" style="background:${oc(r.final)};outline:2px solid #0b0b0b">마지막 답<b>${r.final}</b></div></div>`;
 ...
 h+='<div class="grid" style="margin-top:8px"><div class="h">사실 12개 <span style="font-weight:400;color:#898781;font-size:11px">(색점 = 유리한 선택지 · <span class="sw" style="background:#2a78d6"></span>있음 <span class="sw" style="background:#cde2fb"></span>접힘 · 빈칸 없음)</span></div><div class="h" style="text-align:center">수첩 r0</div><div class="h" style="text-align:center">수첩 r1</div><div class="h" style="text-align:center">수첩 r2</div>';
 ...
 h+=`<div class="kv" style="margin-top:6px">남은 사실(수첩 r0 → r1 → r2): <b>${r.alive.join(' → ')}</b> / 12${r.alive[2]===0?' <span style="color:#b45309">— 마지막 수첩에 "있음"이 0. 접힘(항목 이름만) '+r.grid.filter(g=>g[2]==='접힘').length+'개. 수첩 자체는 아래에 있음('+(r.notes[2]||'').length+'자) — 사실 대신 원칙·결론만 적힌 판</span>':''} · 번복 명시 수첩: ${r.shift.length?r.shift.join(', '):'없음'}${r.jnote?' · 판독 메모: '+r.jnote:''}</div>`;
...
function all(){R.forEach(r=>r.g=GK(r));document.getElementById('bh_model').style.display=mods().length>1?'':'none';document.getElementById('bh_model').textContent='모델';document.getElementById('ntot').textContent=['haiku','gpt','gemini'].map(m=>{const n=R.filter(r=>r.model===m).length;return n?`${m} ${n}판`:''}).filter(Boolean).join(' · ');hero();three();curve();bars('bias',r=>r.inA?[r.inA[2],r.outA[2]]:null,'문장이 편드는 답에 유리한 사실','반대편 사실');bars('bias2',r=>r.proF?[r.proF[2],r.conF[2]]:null,'마지막 답에 유리한 사실','불리한 사실');five();effect();shift();scen();blist()}
